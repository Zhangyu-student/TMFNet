...
class TemporalMambaFusionC1(nn.Module):
    ...
    def forward(self, temporal_features: torch.Tensor, sup: torch.Tensor) -> torch.Tensor:
        ...
        # SSM 门控：也传 conf（1=clean,0=cloudy）
        conf_seq = conf_hw.permute(0, 2, 3, 1).contiguous().view(B * H * W, T, 1)  # [BHW,T,1]
...
class SimpleDecoder(nn.Module):
    ...
    def forward(self, deep: torch.Tensor, skip2: torch.Tensor, skip1: torch.Tensor,
...
# =========================================================
# params & MACs (ptflops) + quick sanity check
# =========================================================
def count_params_m(model: nn.Module) -> float:
    """Trainable parameters in Million (M)."""
    return sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6


def count_macs_g(model: nn.Module, input_res=(3, 3, 256, 256)) -> float:
    """
    MACs in G (1e9). input_res excludes batch, should be (T, C, H, W).
    Requires: pip install ptflops
    """
    try:
        from ptflops import get_model_complexity_info
    except ImportError:
        raise ImportError("ptflops not found. Please run: pip install ptflops")

    model.eval()
    with torch.no_grad():
        macs, params = get_model_complexity_info(
            model,
            input_res=input_res,           # (T,C,H,W) -> ptflops will create [1,T,C,H,W]
            as_strings=False,
            print_per_layer_stat=False,
            verbose=False
        )
    # ptflops returns raw counts
    return macs / 1e9
...
if __name__ == "__main__":
    ...
    # 你想统计的输入尺寸（注意：这里的 input_res 不含 batch）
    T, C, H, W = 3, 3, 256, 256
    input_res = (T, C, H, W)

    # 带云质量头
    net_cqh = TemporalMambaFusionNet(input_nc=3, output_nc=3, use_cloud_head=True).eval()
    params_cqh = count_params_m(net_cqh)
    macs_cqh = count_macs_g(net_cqh, input_res=input_res)
    print(f"[CQH=True ] Params: {params_cqh:.4f} M | MACs: {macs_cqh:.4f} G")

    # 不带云质量头
    net_nocqh = TemporalMambaFusionNet(input_nc=3, output_nc=3, use_cloud_head=False).eval()
    params_nocqh = count_params_m(net_nocqh)
    macs_nocqh = count_macs_g(net_nocqh, input_res=input_res)
    print(f"[CQH=False] Params: {params_nocqh:.4f} M | MACs: {macs_nocqh:.4f} G")

    # （可选）顺手做一次 forward 确认跑通
    x = torch.randn(1, T, C, H, W)
    with torch.no_grad():
        y1, sup1, _ = net_cqh(x)
        y2, sup2, _ = net_nocqh(x)
    print("Forward OK:",
          "with CQH:", y1.shape, sup1[0].shape,
          "| no CQH:", y2.shape, sup2[0].shape)
